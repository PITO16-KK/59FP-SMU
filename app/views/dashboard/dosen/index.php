    ...
    <div class="container">
        <aside class="sidebar">
            ...
            <nav class="menu">
                ...
                <a href="#" class="menu-item" data-page="tips-kesehatan">
                    <i class="fas fa-bullhorn"></i>
                    <span>Tips dan Kesehatan</span>
                </a>

                <div class="menu-separator"></div>

                <a href="#" class="menu-item" data-page="motivasi-page">
                    <i class="fas fa-lightbulb"></i>
                    <span>Manajemen Motivasi</span>
                </a>
        ...
        <main class="main-content">
            ...
            <div id="tips-kesehatan" class="content-section">
                <h1 class="page-title">Tips & Kesehatan</h1>
                <p class="page-subtitle">Panduan menjaga kesehatan saat kualitas udara buruk</p>

                <div class="tips-grid">
                    <div class="tip-card">
                        <div class="tip-icon">😷</div>
                        <h3>Gunakan Masker</h3>
                        <p>Kenakan masker N95 atau masker berkualitas tinggi saat beraktivitas di luar ruangan untuk melindungi dari partikel berbahaya.</p>
                    </div>

                    <div class="tip-card">
                        <div class="tip-icon">🏠</div>
                        <h3>Tetap di Dalam Ruangan</h3>
                        <p>Kurangi aktivitas outdoor saat AQI tinggi. Pastikan ruangan memiliki ventilasi yang baik dan gunakan air purifier jika tersedia.</p>
                    </div>

                    <div class="tip-card">
                        <div class="tip-icon">💧</div>
                        <h3>Banyak Minum Air</h3>
                        <p>Konsumsi air putih yang cukup untuk membantu tubuh mengeluarkan racun dan menjaga fungsi pernapasan tetap optimal.</p>
                    </div>

                    <div class="tip-card">
                        <div class="tip-icon">🏃</div>
                        <h3>Hindari Olahraga Berat</h3>
                        <p>Jangan berolahraga berat di luar ruangan saat kualitas udara buruk. Pilih olahraga ringan di dalam ruangan.</p>
                    </div>

                    <div class="tip-card">
                        <div class="tip-icon">🌿</div>
                        <h3>Tanaman Pembersih Udara</h3>
                        <p>Letakkan tanaman seperti lidah mertua, sirih gading, atau peace lily di rumah untuk membantu membersihkan udara.</p>
                    </div>

                    <div class="tip-card">
                        <div class="tip-icon">🚪</div>
                        <h3>Tutup Jendela</h3>
                        <p>Tutup jendela dan pintu saat kualitas udara luar buruk untuk mencegah polutan masuk ke dalam ruangan.</p>
                    </div>
                </div>

            </div>

            <div id="motivasi-page" class="content-section">
                <h1 class="page-title">Catatan Motivasi</h1>
                <p class="page-subtitle">Inspirasi dan motivasi untuk mahasiswa</p>

                <div class="motivation-card">
                    <div class="motivation-text">"Kesuksesan adalah hasil dari persiapan, kerja keras, dan belajar dari kegagalan."</div>
                    <div class="motivation-author">- Colin Powell</div>
                </div>

                <div class="motivation-card">
                    <div class="motivation-text">"Pendidikan adalah senjata paling ampuh yang bisa kamu gunakan untuk mengubah dunia."</div>
                    <div class="motivation-author">- Nelson Mandela</div>
                </div>

                <div class="card">
                    <h3 style="margin-bottom: 20px;">Tambah Motivasi Baru</h3>
                    <div class="form-group">
                        <label>Kutipan Motivasi</label>
                        <textarea placeholder="Masukkan kutipan motivasi..."></textarea>
                    </div>
                    <div class="form-group">
                        <label>Penulis/Sumber</label>
                        <input type="text" placeholder="Nama penulis...">
                    </div>
                    <button class="btn btn-primary">✨ Tambah Motivasi</button>
                </div>
            </div>

            <script>
                const baseUrl = 'sistem_monitoring_udara/public/motivasi';
                const motivationContainer = document.querySelector('#motivasi-page');

                const kutipanEl = motivationContainer.querySelector('textarea');
                const penulisEl = motivationContainer.querySelector('input[type=text]');
                const btnTambah = motivationContainer.querySelector('.btn-primary');

                let editId = null; // Menyimpan ID motivasi yang sedang diedit

                // ================= Fungsi Load Motivasi =================
                async function loadMotivasi() {
                    try {
                        const res = await fetch(baseUrl);
                        const data = await res.json();

                        // Hapus semua card lama kecuali form
                        motivationContainer.querySelectorAll('.motivation-card').forEach(e => e.remove());
                        ...
                        data.forEach(item => {
                            ...
                            card.innerHTML = `
                <div class="motivation-text">"${item.kutipan}"</div>
                <div class="motivation-author">- ${item.penulis}</div>
                <div style="margin-top:10px;">
                    <button class="edit-btn btn btn-sm btn-warning">Edit</button>
                    <button class="delete-btn btn btn-sm btn-danger">Hapus</button>
                </div>
            `;
                            motivationContainer.insertBefore(card, motivationContainer.querySelector('.card'));
                        });
                    } catch (err) {
                        console.error('Error load motivasi:', err);
                    }
                }

                // ================= Event Delegation =================
                motivationContainer.addEventListener('click', async (e) => {
                    const target = e.target;
                    const card = target.closest('.motivation-card');
                    if (card) {
                        const id = card.dataset.id;

                        // Tombol Edit
                        if (target.classList.contains('edit-btn')) {
                            kutipanEl.value = card.querySelector('.motivation-text').innerText.replace(/"/g, '');
                            penulisEl.value = card.querySelector('.motivation-author').innerText.replace(/^- /, '');
                            editId = id;
                            btnTambah.innerText = '💾 Simpan Perubahan';
                        }
                ...
                btnTambah.onclick = async () => {
                    const kutipan = kutipanEl.value.trim();
                    const penulis = penulisEl.value.trim();
                    if (!kutipan) return alert('Kutipan wajib diisi!');
                    ...
                    if (editId) {
                        ...
                        editId = null;
                        btnTambah.innerText = '✨ Tambah Motivasi';
                    } else {
